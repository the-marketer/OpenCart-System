    ...
    public static function getVariation($byID = null) {
        if (!isset(self::$data['getVariation']) || isset(self::$data['getVariation']) && $byID !== null && self::$data['byID'] !== $byID) {
            ...
            foreach ($productOptions as $val) {
                ...
                unset($optionList[$id][$opt]);
// var_dump($val[$opt]);
                if (is_array($val[$opt])) {
                    foreach ($val[$opt] as $key1 => $val1) {
                        $val[$opt][$key1]['pOption'] = $id;
                    }
                }

                if (!empty($val[$opt]) && is_array($val[$opt])) {
                    $products = self::ProductBuild($val[$opt], $products);
                }
            }
            ...
            foreach ($products as $k => $val) {
                ...
                foreach ($val as $val0) {
                    ...
                    // $newVariation['id'][] = $val0['option_value_id'];
                ...
                // $newVariation['ids'] = implode(Config::$vSeparator, $newVariation['ids']);
